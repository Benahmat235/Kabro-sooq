      ...
      >
        <div className="mx-auto flex max-w-md items-center justify-between px-3">
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1.5 transition-colors focus:outline-none focus:text-primary-700 ${isHome ? 'text-primary-600' : 'text-gray-500 hover:text-gray-700'}`}
          >
          ...
          <button
            ...
            className={`relative flex flex-col items-center justify-center py-1.5 transition-colors focus:outline-none focus:text-primary-700 ${isMessages ? 'text-primary-600' : 'text-gray-500 hover:text-gray-700'}`}
          >
          ...
          <div className="relative -mt-6 flex flex-col items-center justify-center">
            <button
              ...
              className="flex h-13 w-13 items-center justify-center rounded-full bg-gradient-to-tr from-primary-500 to-amber-500 text-white shadow-lg shadow-primary-200 transition-transform active:scale-95 focus:outline-none focus:ring-4 focus:ring-primary-300"
              id="mobile-publish-btn"
            >
              <Plus className="h-7 w-7" aria-hidden="true" />
            </button>
            <span className="text-[10px] font-bold text-primary-600 mt-1">{getTranslation(language, 'publish')}</span>
          </div>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1.5 transition-colors focus:outline-none focus:text-primary-700 ${isMyAds ? 'text-primary-600' : 'text-gray-500 hover:text-gray-700'}`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1.5 transition-colors focus:outline-none focus:text-primary-700 ${isAccount ? 'text-primary-600' : 'text-gray-500 hover:text-gray-700'}`}
          >
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6">
          <div className="flex items-center space-x-2">
            <button
              onClick={() => navigate('/')}
              aria-current={isHome ? 'page' : undefined}
              className={`flex items-center space-x-2 rounded-xl px-4 py-2 text-xs font-bold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 ${
                isHome 
            ...
            <button
              ...
              aria-current={isMessages ? 'page' : undefined}
              aria-label={`${getTranslation(language, 'messages')}${unreadChats > 0 ? `, ${unreadChats} non lus` : ''}`}
              className={`relative flex items-center space-x-2 rounded-xl px-4 py-2 text-xs font-bold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 ${
                isMessages 
                  ? 'bg-primary-600 text-white shadow-md shadow-primary-100' 
                  : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'
              }`}
            >
              ...
              {unreadChats > 0 && (
                <span className={`flex h-4 w-4 items-center justify-center rounded-full text-[9px] font-bold ${
                  isMessages ? 'bg-white text-primary-600' : 'bg-red-500 text-white'
                }`} aria-hidden="true">
                  {unreadChats}
                </span>
              )}
          ...
          <button
            onClick={handlePublish}
            className="flex items-center space-x-2 rounded-xl bg-primary-500 px-5 py-2 text-xs font-bold text-white shadow-md shadow-primary-100 hover:bg-primary-600 hover:shadow-none transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-primary-500"
